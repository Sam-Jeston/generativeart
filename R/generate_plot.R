...
generate_plot <- function(df, file_name, polar, filetype, color, background_color, width, height, dpi) {
  print("generate plot")
  if (polar == TRUE) {
    plot <- df %>%
  ...
  ggplot2::ggsave(plot, width = width, height = height, dpi = dpi, units = "px", filename = paste0(IMG_PATH, file_name), device = filetype, limitsize = FALSE)
  print("image saved...")
}
